...
class MyBLiveClient(blivedm.BLiveClient):
    ...
    async def _on_receive_danmaku(self, danmaku: blivedm.DanmakuMessage):
        ...
        #print(danmaku.msg)
        if danmaku.msg[:2] == r'点歌':
            _id = music.Music.orderMusic(self,danmaku.msg[2:].strip())
            print(danmaku.msg + "   {}".format(danmaku.msg[2:].strip()))
            music.Music.playMusic(self,_id)
...
async def main():
    ...
    client = MyBLiveClient(1027914, ssl=True)
    future = client.start()
    try:
        # 5秒后停止，测试用
